...
import com.test.sockettestclient.logininfo.NoInfoDialog;
...
//        Log.e(TAG, "예약시간 : " + dateFormat.format(workTime.finishWorkTime().getTimeInMillis()) + " || 현재시간 : " + dateFormat.format(System.currentTimeMillis()) );
        ...
        if (ip_text.length() == 0){
            Log.e("AlarmTest", "저장된 데이터가 없습니다. (LoginActivity.class) ");
            NoInfoDialog dialog = new NoInfoDialog(this);
            dialog.show();

        } else {
            DOMAIN = ip_text;
            Log.e("AlarmTest", "저장된 정보 : ip = " + ip_text + " (LoginActivity.class)");
